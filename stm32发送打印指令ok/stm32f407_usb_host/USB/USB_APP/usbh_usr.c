...
#include "ff.h" 
#include "usart.h" 
#include "usmart.h" //my_debug
#include "string.h" //my_debug
//////////////////////////////////////////////////////////////////////////////////	 
                                          ...
                                          USBH_EpDesc_TypeDef *epDesc)
{
	...
	else if((*id).bInterfaceClass==0x07)
	{
		printf("打印机设备\r\n");
	}
}
...
u8 md_send(u8 *buf,u16 length)
{
	...
	if(HCD_IsDeviceConnected(&USB_OTG_Core)&&AppState==USH_USR_FS_TEST)
	{
		...
		USB_Host.buff=buf;
		USB_Host.length=length;
		USBH_MSC_BOTXferParam.BOTState = USBH_MSC_SEND_DATA;
		USBH_MSC_HandleBOTXfer(&USB_OTG_Core ,&USB_Host);
		res=0;
	}
	return res;
