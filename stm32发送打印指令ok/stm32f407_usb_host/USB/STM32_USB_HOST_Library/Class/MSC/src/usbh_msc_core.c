...
{	 
  ...
  //my_debug
  printf("Into USBH_MSC_InterfaceInit\r\n");
  ...
  printf("pphost->device_prop.Ep_Desc[0][0].bEndpointAddress=0x%x\r\n",pphost->device_prop.Ep_Desc[0][0].bEndpointAddress);
  printf("pphost->device_prop.Ep_Desc[0][1].bEndpointAddress=0x%x\r\n",pphost->device_prop.Ep_Desc[0][1].bEndpointAddress);
  ...
  //{
    ...
    else
    {
      ...
      MSC_Machine.MSBulkOutEpSize  = pphost->device_prop.Ep_Desc[0][1].wMaxPacketSize;      
		 ...
		 //my_debug
	  printf("MSC_Machine.MSBulkOutEp=0x%x\r\n",MSC_Machine.MSBulkOutEp);
	  printf("MSC_Machine.MSBulkOutEpSize=0x%x\r\n",MSC_Machine.MSBulkOutEpSize);
     //my_debug
	  printf("MSC_Machine.MSBulkInEp=0x%x\r\n",MSC_Machine.MSBulkInEp);
    printf("MSC_Machine.MSBulkInEpSize=0x%x\r\n",MSC_Machine.MSBulkInEpSize);
    MSC_Machine.hc_num_out = USBH_Alloc_Channel(pdev, 
                                                MSC_Machine.MSBulkOutEp);
    MSC_Machine.hc_num_in = USBH_Alloc_Channel(pdev,
                                                MSC_Machine.MSBulkInEp);  
	//my_debug
		printf("MSC_Machine.hc_num_out=0x%x\r\n",MSC_Machine.hc_num_out);
		printf("MSC_Machine.hc_num_in=0x%x\r\n",MSC_Machine.hc_num_in);
	
    /* Open the new channels */
	//my_debug(change etype from EP_TYPE_BULK to EP_TYPE_INTR )
                                   ...
                                   void   *phost)
{
  ...
  //my_debug
  //printf("into machine:Usbh_msc_core.c\r\n");
  ...
  {   
    ...
    case USBH_MSC_BOT_INIT_STATE:
      USBH_MSC_Init(pdev);
			printf("into Init\r\n");
      //USBH_MSC_BOTXferParam.MSCState = USBH_MSC_BOT_RESET;    //my_debug
    ...
    case USBH_MSC_GET_MAX_LUN:
      ...
      if(status == USBH_OK )
      {
        ...
        if((MSC_Machine.maxLun > 0) && (maxLunExceed == FALSE))
        {
          maxLunExceed = TRUE;
          pphost->usr_cb->DeviceNotSupported();
          printf("into MAX_LUN1\r\n");
          break;
        }
       // USBH_MSC_BOTXferParam.MSCState = USBH_MSC_TEST_UNIT_READY;   //my_debug
				printf("into MAX_LUX2\r\n");
				//USBH_MSC_BOTXferParam.MSCState=USBH_MSC_DEFAULT_APPLI_STATE;
				USBH_MSC_BOTXferParam.MSCState=USBH_GET_CONFIGURATION09;
			}
      ...
      break;
		...
		//mydebug_12.19
		case USBH_GET_CONFIGURATION09:
			if(USBH_Get_CfgDesc(pdev, 
                          phost,
                          USB_CONFIGURATION_DESC_SIZE)==USBH_OK)
				USBH_MSC_BOTXferParam.MSCState=USBH_GET_CONFIGURATION20;
			break;
		
		//mydebug12.19
		case USBH_GET_CONFIGURATION20:
			if(USBH_Get_CfgDesc(pdev, 
                         phost,
                         0x20)==USBH_OK)
				USBH_MSC_BOTXferParam.MSCState=USBH_MSC_DEFAULT_APPLI_STATE;
    ...
    case USBH_MSC_BOT_USB_TRANSFERS:
	  ...
	  //my_debug
	  //printf("USBH_MSC_HandleBOTXfer...\r\n");
      USBH_MSC_HandleBOTXfer(pdev , phost);
      break;
    
    case USBH_MSC_DEFAULT_APPLI_STATE:
			//printf("into MSC State\r\n");
			if(pdev->config==1)
			{
				My_Config(pdev,phost);
			}
			else
			{
				My_Machine(pdev,phost);
			}
			USBH_MSC_BOTXferParam.MSCState = USBH_MSC_DEFAULT_APPLI_STATE;
      /* Process Application callback for MSC */
	  //my_debug
		/*
	  printf("start userApplication\r\n");
...
static USBH_Status USBH_MSC_GETMaxLUN(USB_OTG_CORE_HANDLE *pdev , USBH_HOST *phost)
{
  ...
  phost->Control.setup.b.bRequest = 0x0;
	phost->Control.setup.b.wValue.w = 0;
  phost->Control.setup.b.wIndex.w = 0;
  phost->Control.setup.b.wLength.w = 0x03F1;
	//phost->Control.setup.b.wLength.w = 1;           
  
 // return USBH_CtlReq(pdev, phost, MSC_Machine.buff , 1 ); 
	return USBH_CtlReq(pdev, phost, MSC_Machine.buff , 0x54 ); 
}
